package com.maxminoccheri.rwc.web.rest;
...
import com.maxminoccheri.rwc.IntegrationTest;
import com.maxminoccheri.rwc.config.EmbeddedKafka;
import com.maxminoccheri.rwc.config.KafkaSseConsumer;
import com.maxminoccheri.rwc.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class RemoteWorkCommKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/remote-work-comm-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/remote-work-comm-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/remote-work-comm-kafka/unregister"));
                return;
            }
        }
